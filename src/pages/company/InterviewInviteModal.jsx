...
export function InterviewInviteModal({ applicant, roundNumber, date: initialDate, time: initialTime, onClose, onSend }) {
  ...
  return (
    <>
      ...
      <div ref={modalRef} role="dialog" aria-modal="true" aria-label={`Send Interview ${roundNumber} Invite`} style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 1301, backgroundColor: "white", borderRadius: "1rem", padding: "1.75rem", width: "min(440px,92vw)", boxShadow: "0 24px 64px rgba(0,0,0,0.25)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", marginBottom: "1.1rem", padding: "0.25rem", backgroundColor: "#f1f5f9", borderRadius: "0.6rem" }}>
          {[{ val: "fixed", label: "Fixed Time" }, { val: "slots", label: "Let Student Pick" }].map(({ val, label }) => (
            <button key={val} onClick={() => setMode(val)} style={{ flex: 1, padding: "0.45rem", borderRadius: "0.45rem", border: "none", background: mode === val ? "white" : "transparent", fontWeight: mode === val ? "700" : "500", fontSize: "0.8rem", cursor: "pointer", fontFamily: "inherit", color: mode === val ? "#1e293b" : "#64748b", boxShadow: mode === val ? "0 1px 4px rgba(0,0,0,0.1)" : "none" }}>
              {label}
            </button>
          ))}
        </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                {slots.map((s, i) => (
                  <div key={i} style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
                    ...
                    {slots.length > 1 && <button onClick={() => removeSlot(i)} style={{ padding: "0.3rem 0.5rem", border: "1px solid #fca5a5", borderRadius: "0.4rem", background: "white", color: "#dc2626", cursor: "pointer", fontSize: "0.75rem", fontFamily: "inherit" }}>✕</button>}
                  </div>
                ))}
              </div>
              {slots.length < 5 && (
                <button onClick={addSlot} style={{ marginTop: "0.4rem", padding: "0.35rem 0.75rem", borderRadius: "0.4rem", border: "1.5px dashed #d1d5db", background: "white", color: "#64748b", fontSize: "0.78rem", fontWeight: "600", cursor: "pointer", fontFamily: "inherit" }}>+ Add Slot</button>
              )}
        ...
        <div style={{ display: "flex", gap: "0.6rem", marginTop: "1.25rem" }}>
          <button onClick={onClose} style={{ flex: 1, padding: "0.65rem", borderRadius: "0.6rem", border: "1.5px solid var(--color-border-light, #e2e8f0)", backgroundColor: "var(--color-bg-elevated, white)", color: "var(--color-text-primary, #374151)", fontWeight: "600", fontSize: "0.85rem", cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
