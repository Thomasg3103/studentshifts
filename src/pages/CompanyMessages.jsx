...
function ChatThread({ jobId, studentId, companyId, senderId, studentName, jobTitle }) {
  ...
  useEffect(() => {
    ...
    const filter = isDirect
      ? `and(company_id=eq.${companyId},student_id=eq.${studentId},job_id=is.null)`
...
export default function CompanyMessages() {
  ...
  useEffect(() => {
    ...
    let failed = false;
    Promise.all([
      fetchCompanyConversations(currentUser.id).catch(() => { failed = true; return []; }),
      fetchCompanyDirectConversations(currentUser.id).catch(() => { failed = true; return []; }),
    ]).then(([convs, directs]) => {
              ...
              {directConvs.length === 0
                ...
                : directConvs.map(conv => (
                  <ConvCard
                    ...
                    avatarUrl={conv.profilePhotoUrl}
                    avatarName={conv.studentName}
          ...
          {tab === "jobs" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              {conversations.length === 0
                ? <p style={{ textAlign: "center", color: "#9ca3af", padding: "2rem 1rem", fontSize: "0.875rem" }}>No job chats yet. Accept a student to start chatting!</p>
