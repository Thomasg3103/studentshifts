  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--color-bg-subtle)", padding: "1.5rem 1rem" }}>
      ...
      {modal === "applicants" && activePosting && (
        <div onClick={closeModal} className="applicants-modal-overlay" style={{ position: "fixed", inset: 0, backgroundColor: "rgba(15,23,42,0.55)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem 1rem 5rem", WebkitBackdropFilter: "blur(2px)", backdropFilter: "blur(2px)", animation: "fadeInOverlay 0.18s ease" }}>
          <div ref={applicantsModalRef} onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="applicants-modal-title" className="applicants-modal" style={{ backgroundColor: "var(--color-bg-elevated, white)", borderRadius: "0.85rem", width: "100%", maxWidth: "min(96vw, 1500px)", height: "78vh", maxHeight: "78vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.18)", overflow: "hidden", border: "1px solid var(--color-border-light, #e2e8f0)" }}>
            {/* Header */}
