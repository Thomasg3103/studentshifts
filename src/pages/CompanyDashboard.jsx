...
export default function CompanyDashboard() {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--color-bg-subtle)", padding: "1.5rem 1rem" }}>
      ...
      {/* Tab bar */}
      <div className="tab-scroll-bar" style={{ display: "flex", flexWrap: "nowrap", borderBottom: "1px solid #e2e8f0", marginBottom: "1.75rem", gap: "0", overflowX: "auto", WebkitOverflowScrolling: "touch", touchAction: "pan-x", overscrollBehaviorX: "contain" }}>
        {[
      ...
      {modal === "applicants" && activePosting && (
        <div onClick={closeModal} className="applicants-modal-overlay" style={{ position: "fixed", inset: 0, backgroundColor: "rgba(15,23,42,0.55)", zIndex: 1000, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "5rem 1rem 1.5rem", WebkitBackdropFilter: "blur(2px)", backdropFilter: "blur(2px)", animation: "fadeInOverlay 0.18s ease" }}>
          <div ref={applicantsModalRef} onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="applicants-modal-title" className="applicants-modal" style={{ backgroundColor: "var(--color-bg-elevated, white)", borderRadius: "0.85rem", width: "100%", maxWidth: "min(96vw, 1500px)", height: "72vh", maxHeight: "72vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.18)", overflow: "hidden", border: "1px solid var(--color-border-light, #e2e8f0)" }}>
            {/* Header */}
