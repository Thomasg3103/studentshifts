                  ...
                  {!list.length ? <p style={{ color: "#94a3b8", fontSize: "0.875rem" }}>No posts match "{postSearch}"</p> : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                      {list.map(post => {
                        ...
                        const rawPhoto   = post.photos?.[0] || null;
                        const photo      = supabaseImg(rawPhoto, 400);
                        const crop       = post.photo_crops?.[0] || { zoom: 1, offsetX: 0, offsetY: 0 };
                        return (
                          <div key={post.id} style={{ border: isReported ? "2px solid #fca5a5" : "1.5px solid #e2e8f0", borderRadius: "1rem", overflow: "hidden", backgroundColor: isReported ? "#fff5f5" : "white", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                            ...
                            {/* Job card — same layout as Student Dashboard */}
                            <div className="job-card" style={{ display: "flex", flexDirection: "row", alignItems: "stretch", padding: 0, margin: "0.3rem", borderRadius: "0.75rem", overflow: "hidden", cursor: "pointer", border: "none", background: "transparent" }} onClick={() => setExpandedPostId(isExpanded ? null : post.id)}>
                              {/* Photo side */}
                              <div style={{ width: "130px", flexShrink: 0, alignSelf: "stretch", position: "relative", overflow: "hidden", borderRadius: "0.75rem 0 0 0.75rem", minHeight: "110px" }}>
                                {photo ? (
                                  <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, transform: `translate(${crop.offsetX}%, ${crop.offsetY}%) scale(${crop.zoom})`, transformOrigin: "center" }}>
                                    <img loading="lazy" src={photo} alt={post.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                                  </div>
                                ) : (
                                  <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #fce7f3 0%, #f5d0e3 100%)" }}>
                                    <span style={{ fontSize: "1.8rem", fontWeight: 800, color: "var(--color-brand)", opacity: 0.7 }}>{(post.companyName || "?").slice(0, 2).toUpperCase()}</span>
                                  </div>
                                )}
                              </div>
                              {/* Content side */}
                              <div style={{ flex: 1, padding: "0.7rem 0.9rem", minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", gap: "0.25rem" }}>
                                <div>
                                  <h2 style={{ fontWeight: 800, fontSize: "1rem", margin: "0 0 0.1rem", color: "var(--color-text-primary, #1e293b)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{post.title}</h2>
                                  <p style={{ margin: "0 0 0.3rem", fontSize: "0.8rem", color: "var(--color-text-secondary, #6b7280)" }}>{post.companyName}</p>
                                  <p style={{ margin: "0 0 0.35rem", fontSize: "0.72rem", color: "#64748b" }}>📍 {post.location}</p>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap", marginTop: "0.2rem" }}>
                                  <span style={{ fontWeight: 800, color: "var(--color-brand)", fontSize: "1.1rem" }}>
                            ...
                            {/* Report pills */}
                            {isReported && (
                              <div style={{ margin: "0 0.75rem 0.5rem", display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                                {post.reports.map(r => (
                                    ...
                                    {r.screenshot_urls?.length > 0 && (
                                      <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.4rem", flexWrap: "wrap" }}>
                                        {r.screenshot_urls.map((url, i) => (
                                          <a key={i} href={url} target="_blank" rel="noreferrer"><img src={url} alt="screenshot" style={{ width: "64px", height: "64px", objectFit: "cover", borderRadius: "0.3rem", border: "1px solid #fca5a5" }} /></a>
                                        ))}
                                      </div>
                                    )}
                                  </div>
                            ...
                            {isExpanded && post.description && (
                              <div style={{ margin: "0 0.75rem 0.5rem", borderTop: "1px solid #e2e8f0", paddingTop: "0.65rem" }}>
                                <div style={{ fontSize: "0.82rem", color: "#1e293b", lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: post.description }} />
                              </div>
                            )}

                            {/* Delete */}
                            <div style={{ padding: "0 0.75rem 0.65rem", display: "flex", justifyContent: "flex-end" }}>
                              <button onClick={() => handleAdminDeletePost(post.id, post.title)} disabled={deletingPostId === post.id} style={{ padding: "0.3rem 0.65rem", borderRadius: "0.4rem", border: "1.5px solid #fca5a5", backgroundColor: "#fff1f2", color: "#e11d48", fontWeight: "700", fontSize: "0.75rem", cursor: "pointer", fontFamily: "inherit" }}>{deletingPostId === post.id ? "…" : "🗑 Delete"}</button>
                            </div>
                          </div>
                        );
                      })}
